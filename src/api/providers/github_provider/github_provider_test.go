...
func TestCreateRepoUnouthorized(t *testing.T) {
	restclient.FlushMockups()
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123,"name": "golang-tutorial","full_name": "rjunior/golang-tutorial"}`)),
		},
	})
	response, err := CreateRepo(config.GetGithubAccessToken(), github.CreateRepoRequest{})
	assert.Nil(t, err)
	...
	assert.EqualValues(t, "rjunior/golang-tutorial", response.FullName)
}
